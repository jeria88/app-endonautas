def evaluate_test(test_name, details):
    ...
    elif 'Tríada Oscura' in test_name or 'Dirty Dozen' in test_name:
...
def _eval_big_five(details):
    ...
    if dimensiones:
        ...
        conclusion = (
            f"Tu rasgo dominante ({sd[-1]['nombre']}) es tu mayor ventaja evolutiva y también tu zona de confort automática. "
...
def _eval_jung(details):
    ...
    return {
        ...
        "conclusion": (
            "La tipología de Jung nos enseña que el equilibrio no es ser 'un poco de todo', "
...
def _eval_eneagrama(details):
    ...
    return {
        "dimensiones": dimensiones,
        "conclusion": (
            "El eneagrama describe las 9 formas de 'olvidar' quiénes somos. Tu tipo dominante es tu mayor "
...
def _eval_gad7(details):
    ...
    conclusion = (
        "La ansiedad no es un error de tu sistema — es una señal. En términos endonáuticos, la ansiedad crónica "
...
def _eval_phq9(details):
    ...
    conclusion = (
        "La depresión a menudo funciona como un mecanismo de apagado del sistema biológico cuando hemos estado "
...
def _eval_pss10(details):
    ...
    conclusion = (
        "El estrés no es lo que te sucede, sino cómo tu biología y psicología interpretan lo que te sucede "
...
def _eval_maia(details):
    ...
    conclusion = (
        "La interocepción es la base de toda inteligencia emocional. El MAIA no evalúa tu 'condición física', "
...
def _eval_neurosensorial(details):
    ...
    conclusion = (
        "Tu perfil neurosensorial es tu 'hardware' original. Muchas personas creen que tienen 'problemas de ansiedad' "
...
def _eval_svi(details):
    ...
    conclusion = (
        "La vitalidad subjetiva es el termómetro de tu alineación interna. Cuando tu energía fluye sin resistencias "
...
def _eval_ecr(details):
    ...
    conclusion = (
        f"Basado en tus coordenadas, tu estilo predominante es {estilo}. "
...
# ─────────────────────────────────────────────────────────────────────
# HERIDAS DE LA INFANCIA (Bourbeau)
# ─────────────────────────────────────────────────────────────────────
...
def _eval_ibi(details):
    max_por_dim = {
        ...
        'Perfeccionismo': 10,
        'Culpa y Condena': 5,
        'Intolerancia a la Frustración': 10,
        'Irresponsabilidad Emocional': 5,
        'Ansiedad Ansiosa': 5,
        'Evitación': 5,
    }
    dimensiones = []
    ...
    for dim, score in details.items():
        ...
        if pct > worst_pct:
            ...
            worst_dim = dim
    conclusion = (
        "La terapia racional-emotiva de Ellis postula que el sufrimiento no viene de lo que te pasa, "
...
def _eval_autosabotaje(details):
    max_por_dim = {
        'Procrastinación': 15,
        'Perfeccionismo Paralizante': 10,
        'Miedo al Éxito': 10,
        'Síndrome del Impostor': 10,
    }
    dimensiones = []
    for dim, score in details.items():
    ...
    conclusion = (
        "El autosabotaje no es falta de 'disciplina'; es tu sistema psíquico protegiéndote de un dolor percibido mayor. "
...
def _eval_dirty_dozen(details):
    ...
    conclusion = (
        "Nadie está compuesto puramente de luz. La Tríada Oscura evalúa los mecanismos evolutivos de supervivencia predatoria "
...
def _eval_tas20(details):
    ...
    for dim, score in details.items():
        ...
        pct = min((score / max_s) * 100, 100)
        nivel = "Alto" if pct > 70 else "Moderado" if pct > 40 else "Bajo"
        pol, msg, acc = _get_polarity(dim, pct, es_interferencia=True)
        dimensiones.append({
            'nombre': dim, 'puntos': score, 'max': max_s, 'pct': pct,
            'polaridad': pol, 'mensaje_polaridad': msg, 'accion_sugerida': acc,
    ...
    nivel_total = "Sin alexitimia" if total_score <= 51 else "Posible alexitimia" if total_score <= 60 else "Alexitimia"
    conclusion = (
        f"Puntuación total TAS-20: {total_score}/100 — {nivel_total} (Bagby et al., 1994; cortes: ≤51 / 52-60 / ≥61). "
...
def _eval_ders(details):
    ...
    conclusion = (
        "La regulación emocional es la capacidad de surfear la ola del sentimiento sin ser arrastrado por ella. "
...
def _eval_chakras(details):
    ...
    conclusion = (
        "El sistema de chakras es un mapa de la consciencia humana. Un chakra 'bloqueado' es simplemente una parte de tu experiencia vital "
...
def _eval_swb(details):
    ...
    conclusion = (
        "El bienestar espiritual es el equilibrio entre tu conexión con lo sagrado y tu propósito en la tierra. "
...
def _eval_cloninger(details):
    max_por_dim = {
        'Absorción Transpersonal': 10,
        'Identificación Mística': 10,
        'Aceptación Espiritual': 5,
    }
    dimensiones = []
    for dim, score in details.items():
        max_s = max_por_dim.get(dim, 10)
    ...
    conclusion = (
        "La autotrascendencia es la capacidad de disolver las fronteras del ego y sentirse parte de una totalidad mayor. "
...
def _eval_logotest(details):
    max_por_dim = {
        'Sentido': 10,
        'Vacío Existencial': 10,
        'Resiliencia de Sentido': 5,
    }
    dimensiones = []
    for dim, score in details.items():
        max_s = max_por_dim.get(dim, 5)
    ...
    conclusion = (
        "La voluntad de sentido es la motivación primaria del ser humano (Frankl). "
...
def _eval_dri_das(details):
    ...
    conclusion = (
        "Tu relación con tus sueños es un puente hacia tu inconsciente. "
...
def _eval_lucidez(details):
    ...
    conclusion = (
        "La lucidez onírica es el arte de despertar dentro del sueño. Expande la consciencia y permite "
...
def _eval_via(details):
    ...
    conclusion = (
        "Tus fortalezas de carácter son tus 'herramientas de fábrica'. Usarlas conscientemente es el predictor "
...
def _eval_riasec(details):
    ...
    conclusion = (
        "El modelo RIASEC sugiere que prosperamos en entornos que coinciden con nuestra personalidad vocacional. "
...
def _eval_mwq(details):
    ...
    conclusion = (
        "El trabajo deja de ser una carga cuando se convierte en una vía de expresión de tus valores. "
...
def _eval_kolb(details):
    max_por_dim = {
        'Experiencia Concreta': 10,
        'Observación Reflexiva': 10,
        'Conceptualización Abstracta': 5,
        'Experimentación Activa': 5,
    }
    dimensiones = []
    for dim, score in details.items():
        max_s = max_por_dim.get(dim, 5)
    ...
    conclusion = (
        "Aprender a aprender es la habilidad maestra. Tu estilo de Kolb te dice cómo procesas mejor la realidad: "
...
def _eval_ceq(details):
    max_por_dim = {
        'Curiosidad Epistémica': 15,
        'Tolerancia a la Ambigüedad': 5,
    ...
    conclusion = (
        "La curiosidad es el motor de la expansión. Mantener el 'hambre de saber' mantiene la mente joven y plástica. "
...
def _eval_comunidad(details):
    max_por_dim = {
        'Apoyo Emocional': 15,
        'Apoyo Informacional': 10,
        'Apoyo Instrumental': 10,
        'Apoyo de Compañía': 5,
    }
    dimensiones = []
    for dim, score in details.items():
    ...
    conclusion = (
        "Somos seres tribales. Tu salud biológica y mental está íntimamente ligada a la calidad de tu red social. "
        "No se trata de tener muchos contactos, sino de tener provisiones sociales claras: apego, integración y valor social. "
        "⚠️ Adaptación de 8 ítems del MOS-SSS (20 ítems originales)."
    )
    return {"dimensiones": dimensiones, "conclusion": conclusion}


# ─────────────────────────────────────────────────────────────────────
# FORTALEZAS PROSOCIALES
# ─────────────────────────────────────────────────────────────────────
...
def _eval_comunicacion(details):
    max_por_dim = {
        ...
        'Escucha Activa': 5,
        'Empatía': 5,
        'Resolución Colaborativa': 5,
        'Expresión Emocional': 5,
    }
    dimensiones = []
    for dim, score in details.items():
        max_s = max_por_dim.get(dim, 5)
    ...
    conclusion = (
        "La asertividad es la capacidad de honrar tu verdad sin deshonrar la del otro. "
...
def _eval_maq(details):
    max_por_dim = {
        'Creencias Limitantes': 10,
        'Merecimiento': 5,
        'Claridad Financiera': 5,
        'Flujo': 5,
        'Mentalidad de Escasez': 5,
    }
    dimensiones = []
    for dim, score in details.items():
        max_s = max_por_dim.get(dim, 5)
    ...
    conclusion = (
        "Tu relación con el dinero es un espejo de tu relación con la vida. "
...
def _eval_fss(details):
    ...
    conclusion = (
        "Las finanzas estresadas revelan la relación con la abundancia, el merecimiento y el control. "
...
def _eval_creatividad(details):
    ...
    conclusion = (
        "La creatividad no es una habilidad, es una forma de ver. Cuando integras tu identidad creativa, "
...
def _eval_integracion(details):
    ...
    conclusion = (
        "La integración no es perfección en todas las áreas — es la capacidad de moverte conscientemente entre ellas. "
...
def _eval_soc29(details):
    ...
    conclusion = (
        "El Sentido de Coherencia (Antonovsky) es lo que te permite navegar el caos sin perder el centro. "
