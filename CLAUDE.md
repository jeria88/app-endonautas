...
| `tokens` | 🔴 legacy | Fractones desactivados. Desde 2026-07-29 las rutas `/tokens/` ya NO están montadas y payments apunta a `reverse('planes')`. Queda cableado solo lo contable: `accounts/signals.py` crea TokenBalance en cada alta y los webhooks llaman `renew_monthly()` — saldo que nadie gasta ni muestra. Los modelos se conservan por las compras históricas. |
...
**Sistema prompt:** `mirror/prompts/espejo_system.txt` — cargado en cada request vía `_load_system_prompt()`. Reestructurado 2026-07-06 como arquitectura de modos (antes: pila de parches): IDENTIDAD + LÍMITES DUROS invariantes → dispatcher con precedencia estricta **CRISIS > SÍNTOMA FÍSICO > REVELACIÓN (default)**. No existe modo "sostén" — el Espejo siempre entrega lectura (reconocimiento 1 línea + patrón/hipótesis + invitación a confirmar). Las reglas de tono viven dentro de cada modo, no globales. En CRISIS se suspende lectura simbólica y marco teórico. Regla al editar: todo cambio de comportamiento entra en el modo que corresponde, nunca como sección suelta al final.

**Contexto inyectado:** `user_intent_context()` (onboarding_priorities) + `user_history_context()` (tests psicométricos, sesión Espejo anterior, bitácora, lectura de nacimiento) + `_retrieve_context()` (RAG: top-5 chunks de KnowledgeChunk por similitud coseno o keyword). Se ensambla identidad-primero: system prompt + `<contexto_usuario>` + `<marco_teorico>` (bloques delimitados que el prompt referencia por nombre). Implementado en `config/ai_client.py` y `mirror/views.py::_get_reply()`. `max_tokens` = 1000.

**Memoria entre sesiones (activa desde 2026-07-06):** `_summarize_previous_session()` en `mirror/views.py` — al crear sesión nueva (`chat_new`), la última sesión previa con ≥4 mensajes y sin resumen recibe `conflict_summary` + `return_question` vía una llamada IA. `user_history_context()` los inyecta como "Sesión anterior del Espejo".
